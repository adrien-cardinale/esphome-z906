...
void Z906Component::set_adc_pins(int console_presence_pin, int amplifier_presence_pin, int alim_pin) {
...
adc_oneshot_unit_handle_t Z906Component::get_adc_unit_handle_(adc_unit_t unit) {
  adc_oneshot_unit_handle_t &handle = (unit == ADC_UNIT_1) ? this->adc1_handle_ : this->adc2_handle_;
  if (handle == nullptr) {
...
void Z906Component::init_adc_channel_(AdcChannel &adc, int gpio_pin) {
  ...
  if (adc_oneshot_config_channel(adc.unit_handle, adc.channel, &chan_cfg) != ESP_OK) {
  ...
  if (adc_cali_create_scheme_curve_fitting(&cali_cfg, &adc.cali_handle) != ESP_OK) {
    ESP_LOGW(TAG, "ADC calibration unavailable for GPIO%d, falling back to raw scaling", gpio_pin);
...
float Z906Component::read_adc_voltage_(AdcChannel &adc) {
  ...
  if (adc.cali_handle != nullptr) {
    int millivolts;
    if (adc_cali_raw_to_voltage(adc.cali_handle, raw, &millivolts) == ESP_OK)
...
void Z906Component::setup() {
  this->init_adc_channel_(this->console_presence_adc_, this->console_presence_pin_);
  this->init_adc_channel_(this->amplifier_presence_adc_, this->amplifier_presence_pin_);
...
void Z906Component::updatePresence() {
    ...
    if (now - this->last_presence_update_ < PRESENCE_UPDATE_INTERVAL_MS)
        return;
    this->last_presence_update_ = now;

    const float voltage_console_presence = this->read_adc_voltage_(this->console_presence_adc_);
    const float voltage_amplifier_presence = this->read_adc_voltage_(this->amplifier_presence_adc_);
    const float voltage_alim = this->read_adc_voltage_(this->alim_adc_);

    if (voltage_console_presence < 0.1f && voltage_amplifier_presence < 0.1f && voltage_alim > 3.0f) {
        this->stable = true;
    } else {
        this->stable = false;
    }
